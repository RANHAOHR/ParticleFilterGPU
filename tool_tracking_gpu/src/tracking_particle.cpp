#include <ros/ros.h>
#include <tracking_particle_gpu_lib/particle_filter.h>
...
int main(int argc, char **argv) {
	...
	ros::NodeHandle nh;
	/******  initialization  ******/
	ParticleFilterGPU ParticleFilterGPU(&nh);
	...
	while (nh.ok()) {
        ...
        if (freshImage) {
            ...
            ParticleFilterGPU.raw_image_left = rawImage_left.clone();
            ParticleFilterGPU.raw_image_right = rawImage_right.clone();
            ...
            ParticleFilterGPU.trackingTool(seg_left, seg_right); //with rendered tool and segmented img
